...
public:
    ...
    bool handleEvent(const sf::Event& event) override {
        if ((event.type == sf::Event::EventType::KeyPressed) &&
            ...
            pong_screen->load();
            ScreenService::get().addScreen(static_cast<Screen*>(pong_screen));
            close();
        }
    ...
    void update(const sf::Time& time) override {
        if (getIsEntering() || getIsExiting()) return;
    ...
    void onEntering(const float transitionInterpolation) override {
        vec2 pos = interpolate(title_from, title_to, transitionInterpolation);
        title.setPosition(pos.x, pos.y);
        pos = interpolate(press_start_from, press_start_to, transitionInterpolation);
        press_start.setPosition(pos.x, pos.y);
    }

    void onExiting(const float transitionInterpolation) override {
        vec2 pos = interpolate(title_to, title_from, transitionInterpolation);
        title.setPosition(pos.x, pos.y);
        pos = interpolate(press_start_to, press_start_from, transitionInterpolation);
        press_start.setPosition(pos.x, pos.y);
    }
